...
| 主数据库 | IndexedDB + Dexie | 保存卡片、频道、标签、正文和图片 Blob |
...
├── domain/                # InspirationItem、Channel、Tag
...
└── features/              # 收藏、频道、标签、搜索等功能
```

### 扩展页面的职责

- **Popup**：收藏当前页，呈现即时成功或失败状态，允许输入备注。
- **Dashboard**：左侧频道、顶部搜索和筛选、瀑布流卡片墙、居中详情灯箱和沉浸式图片浏览。
...
搜索结果同时受频道、标签、时间和处理状态过滤。频道和标签的精确过滤直接由 Dexie 查询或内存集合完成，全文相关性交给 MiniSearch。
...
- 搜索、频道、筛选和当前详情项：URL search params，便于返回时恢复上下文。
...
4. 重复收藏不生成重复卡片。
5. 导入书签 HTML 和多个 URL。
6. 搜索、频道筛选和居中详情灯箱保留当前上下文与滚动位置。
7. 点击卡片非交互区域打开详情；标签、收藏、外链和更多菜单不会误触父级行为。
8. 图片模式按一次 `Escape` 返回详情、再次按下关闭灯箱，关闭后焦点回到原卡片。
9. 编辑备注后切换项目、关闭灯箱或打开原网页，内容均已写入 Dexie。
10. `prefers-reduced-motion` 下灯箱和卡片不执行位移、缩放或背景模糊动画。
