...
- 全项目使用协调的平滑圆角：收藏卡片（含紧凑卡片）与收藏项详情窗口使用 Lisse `0.6` 精确曲线；Chrome/Edge 其余圆角表面使用原生 `corner-shape: superellipse(1.6)`。Lisse `0.6` 沿边延伸约为基础半径的 `1.6` 倍，因此支持 `corner-shape` 的浏览器把基础 CSS 半径同步放大 `1.6` 倍，避免按钮、输入框及浮层看起来比 Lisse 卡片更方；不支持时仍使用原半径的普通圆角。原生曲线使动态描边、阴影和焦点轮廓随形状绘制，避免逐控件使用 Lisse 裁切时丢失这些状态。圆形头像、状态点、单选指示器、关闭按钮及添加主按钮保持正圆。卡片描边、悬停阴影、键盘焦点、瀑布流测高及详情内的标签候选浮层不得因平滑圆角失效。
...
- 快捷视图只预置不可编辑、不可删除、不可拖动的“稍后阅读”。用户新增的视图只记录侧栏范围与标签组合，不记录排序方式或排列样式；默认名称按“新快捷视图 1、2、3…”递增，旧的同名默认视图在数据库升级时自动补上序号。在悬停或键盘聚焦时显示更多按钮。鼠标从条目进入更多按钮时，条目的悬停背景必须持续显示。普通图标按钮（包括新增、更多、关闭、编辑、方向切换和星标）悬停时统一使用 `surface-subtle`，图标加深为 `ink`；当前选中项可保留品牌色图标，但背景仍使用 `surface-subtle`。危险图标按钮使用危险红图标和 `danger-bg` 悬停背景。快捷视图支持行内改名、二次确认删除和原生拖拽排序，但管理菜单不显示拖拽说明项；系统视图始终位于首位。侧栏选中项的图标、文字与数量统一使用品牌主色。
