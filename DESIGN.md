...
- 参考 [Libraries.dev Beam 的暗色设计系统](https://libraries.dev/beam)（2026-09-26 核对其 `site.css`）：页面基底 `#121212`，主表面 `#181818`，次级交互层使用略亮的深灰，主要文字亮白、说明文字分级灰。只借鉴明度层级；Webloom 继续使用已确认的 P3 品牌橙，不移植 Beam 的蓝色强调色、字体或组件造型。
- 暗色语义值集中在 `src/design-system/theme.css`，先给 sRGB 回退，再在 `@supports (color: color(display-p3 1 1 1))` 内提供同名 P3 权威值。关键核对值：`canvas #121212`、`surface #181818`、`surface-secondary #202020`、`surface-subtle #262626`、`border-subtle #303030`、`ink #F5F5F5`、`text #DEDEDE`、`muted #B1B1B1`。状态色在暗背景下提亮，柔和状态底色压暗；标签文字改用高对比橙，主按钮仍使用原品牌橙。
- `html[data-theme]` 控制语义 token 和浏览器 `color-scheme`；外观设置提供“系统 / 浅色 / 深色”三选一，选择保存在 `chrome.storage.local`。未设置时默认跟随系统，并实时响应 `prefers-color-scheme` 变化。页面、设置、设计系统预览与快速收藏浮层共用当前主题；开发预览的色值标签随实际主题切换。
- 主题选择区参考 [Adi 的 Theme switcher 视频](https://x.com/AdityaSur11/status/2103024134607282215/video/1)，仅借鉴切换区：三张并排的微型工作台缩略图按“深色 / 浅色 / 系统”排列，系统卡从左暗到右亮对半展示；只有当前选中卡下方显示名称。选中卡用当前主题的 `ink` 细描边，键盘焦点仍使用品牌橙焦点环；不引入视频中的弹窗标题、关闭按钮或保存按钮。预览中的中间灰（如 `#5A5A5A`）只用于静态示意，不是应用语义色。

## 布局

- 桌面侧栏默认宽度 `220px`，可通过右侧拖拽在 `176–320px` 之间调整；松开时接近默认宽度 `±20px` 会吸附回 `220px`。侧栏保持白底，右侧使用 `border-subtle #eee9e3` 细分隔线。
- 侧栏品牌图标为 `30px`，Wanderland 字标为 `22px`、深灰色、`600` 字重；普通导航文字为 `14px`，未选中导航图标使用比文字浅一级的 `muted` 色；选中项使用中性灰背景和深灰文字、图标，不使用品牌色。窄屏折叠侧栏时品牌图标仍为 `30px`，文字隐藏。
...
- 已选标签显示在工具栏下一行，不显示 `#` 前缀，每个标签都可通过 `X` 单独移除。已选标签使用白色表面、三级文字色和全圆角外形，与暖灰页面形成清楚层次；标签超过当前屏幕可用宽度时只展示部分标签，并提供“+N / 收起”按钮切换完整展示。
...
- 卡片模式遵循 Figma「卡片组件 / 卡片」：网站显示封面、标题、描述与最多 3 个核心标签；文章不显示封面，正文区域根据实际内容自适应高度；X 帖子将作者显示在来源行，以正文首句作为标题；关注源使用 `40px` 头像突出作者。标题使用 `16px / 22px`、600 字重并最多两行，描述使用 `13px / 20px` 并最多两行。紧凑卡片沿用原横向卡片：网站封面宽 `142px`，文章使用纯文本，关注源使用 `60px` 头像；按容器宽度呈现两列或一列瀑布流。列表模式遵循 Figma「列表组件 / 列表 item」，不显示描述；网站仅在有真实 OG 图时显示 `128×72px` 封面，文章使用纯文本，关注源使用 `64px` 头像。
- 用户填写、网页自带与 AI 生成的描述在卡片和紧凑卡片中共用同一视觉位置和排版，不显示来源 Badge；列表不显示描述。描述优先级只影响内容选择，不增加卡片复杂度。所有标签组件统一显示纯标签名称，不添加 `#` 前缀；卡片和列表最多显示 3 个标签，完整标签在详情和管理界面中查看。标签使用共享 Badge 的 `neutral` variant：默认态为 `surface-secondary` 背景、`text-tertiary` 文字、`border-default` 细描边和全圆角；悬停或键盘聚焦时切换为 `accent-primary-bg` 背景和 `accent-primary` 文字，并隐藏默认灰色内描边。详情列表的列表项悬停或内部控件获得焦点时，未被直接操作的标签背景切换为白色；具体标签悬停或获得键盘焦点时仍使用统一的主色交互态。点击筛选和多标签组合逻辑保持不变。
- 详情卡片网格默认显示三列，内容区宽度达到 `1200px` 时切换为四列；不足三列的视口逐级收至两列和单列。紧凑卡片保持现有列数。水平与垂直卡片间距均为 `8px`，卡片边缘、封面、信息和标签的内部间距规则保持 `8px`。
...
- 全项目使用协调的平滑圆角：收藏卡片（含紧凑卡片）与收藏项详情窗口使用 Lisse `0.6` 精确曲线；Chrome/Edge 其余圆角表面使用原生 `corner-shape: superellipse(1.6)`。Lisse `0.6` 沿边延伸约为基础半径的 `1.6` 倍，因此支持 `corner-shape` 的浏览器把基础 CSS 半径同步放大 `1.6` 倍，避免按钮、输入框及浮层看起来比 Lisse 卡片更方；不支持时仍使用原半径的普通圆角。原生曲线使动态描边、阴影和焦点轮廓随形状绘制，避免逐控件使用 Lisse 裁切时丢失这些状态。关注源头像保持 `42%` 大圆角且随尺寸同比缩放；状态点、单选指示器、关闭按钮及添加主按钮保持正圆。卡片描边、悬停阴影、键盘焦点、瀑布流测高及详情内的标签候选浮层不得因平滑圆角失效。
...
- 卡片站点图标与内容封面属于收藏内容，不属于通用图标系统。来源行的 favicon 以原始图片或 SVG 形状显示在 `16×16px` 区域，不统一加圆角、边框、内边距或阴影。关注源头像在卡片、紧凑卡片、列表中分别为 `40px`、`60px`、`64px`，统一使用一圈浅色细边框与近圆形大圆角，边框内的留白随尺寸由 `2px` 增至 `3px`；图片不叠加第二圈白边或阴影。站点图标优先使用页面声明并采集保存的 favicon；缺失或加载失败时尝试站点根目录 `/favicon.ico`，仍失败则显示 Remix Icon 通用网站占位，不允许留下空白。采集时仅对已授权同域、可解码的图标做本地像素分析：浅色透明标志自动使用深色底，其他图片保持原样；分析失败不影响收藏。详情编辑提供“图标底色：自动 / 浅底 / 深底”的预览与人工修正，人工设置优先于自动分析。内容封面应使用设计稿导出的实际资源并保存在 `public/assets/`，或使用收藏数据中的真实资源。
- 详情编辑中的“图标底色”与其他字段保持同一阅读顺序：标题单独占一行，下方左侧为当前图标预览，右侧为底色选项及说明；窄屏时说明可以换行，但预览和选项仍保持左右对应。
...
- 全局成功、信息、警告、错误与等待状态分别统一使用绿、蓝、橙、红、黄五组语义 token，状态文字与反馈表面共用同一套色彩，不在组件内创建近似状态色。所有 Toast 统一由共享 `Toast` 组件渲染：固定在工作台顶部居中，使用无图标的白色胶囊、宽扩散阴影与按内容收缩的宽度；仅有标题时保持单行并在过长时省略，有描述时才展开为双行信息卡，与是否包含操作按钮无关。成功 Toast 的标题无论是否带描述都使用 `ink` 黑色文字 token；信息、警告和错误 Toast 分别保持蓝、橙、红语义色，中性通知使用次级灰。可撤回通知在次级灰操作文字后显示深灰收藏项名称。所有 Toast 的操作按钮均使用 `ink` 深色胶囊，可撤回按钮额外显示当前平台的 `⌘Z` 或 `Ctrl+Z`，快捷键在文本输入时保留原生撤销行为。自动消失的通知不常驻显示关闭按钮，持续型 loading 必须保留关闭入口。通知最多同时显示两条，其余排队；默认通知显示 4 秒，带操作通知显示 7 秒，悬停、键盘聚焦或页面进入后台时暂停计时；`Alt+T` 可将焦点移入通知区域，`Escape` 关闭当前通知，`prefers-reduced-motion` 下取消位移、缩放与模糊过渡。
...
- 设置页仅在用户切换左侧功能项后，让右侧内容区整体执行 Cube Motion `rise`；初次进入设置页不播放。收藏项详情保留 React Aria Modal 原有开关与焦点行为，仅在详情内部滚动区对封面、标题、标签和后续信息块执行 Cube Motion `reveal`；编辑表单不执行 reveal，以免影响自动聚焦。两处动效都遵循 `prefers-reduced-motion`，不将动效应用到侧栏或弹窗遮罩。
- 液态形变只用于添加入口，使用 `liquid-gooey` 保持真实 DOM 按钮；三个操作在收起时不得进入 Tab 顺序。键盘可用 Enter/Space 展开、Escape 收起，`prefers-reduced-motion` 下取消弹性位移。
- 标签统一通过共享 Badge 渲染且不显示 `#` 前缀。强调标签使用 `tag-text` 常规字重文字与 `tag-surface` 背景；卡片和列表中的内容标签使用 `neutral` variant，标签之间保持 `8px` 间距。侧栏选中项不使用品牌色，使用比悬停表面略深的 `sidebar-active` 中性灰背景；图标、文字保持深灰，数量使用次级灰。首页和设置页导航项统一为 `36px` 最小高度；首页在分组内保持紧凑行距并以 `16px` 分组间距建立节奏，设置导航的选项间距为 `8px`。
- 快捷视图只预置不可编辑、不可删除、不可拖动的“稍后阅读”。用户新增的视图只记录侧栏范围与标签组合，不记录排序方式或排列样式；默认名称按“新快捷视图 1、2、3…”递增，旧的同名默认视图在数据库升级时自动补上序号。在悬停或键盘聚焦时显示更多按钮。鼠标从条目进入更多按钮时，条目的悬停背景必须持续显示。普通图标按钮（包括新增、更多、关闭、编辑、方向切换和星标）悬停时统一使用 `surface-subtle`，图标加深为 `ink`；当前选中项可保留品牌色图标，但背景仍使用 `surface-subtle`。危险图标按钮使用危险红图标和 `danger-bg` 悬停背景。快捷视图支持行内改名、二次确认删除和拖拽排序，但管理菜单不显示拖拽说明项；系统视图始终位于首位。拖动排序参考 [Jamie Merrill 的 pinned pages 视频](https://x.com/Jamie_Merrill/status/2099719860070559859)：指针移动超过小阈值后才进入排序，拖动项跟手，越过其他项中线时即预览新顺序，邻项用快速减速位移让位，松手后落位并保存；按 Escape 或在列表外松手时恢复原顺序。排序不调用浏览器原生 HTML 拖放，避免系统拖拽图标与自绘跟手项叠加。`prefers-reduced-motion` 下邻项立即换位，不执行补间。侧栏选中项的图标、文字与数量统一使用品牌主色。
...
- 收藏项详情中的编辑在当前详情布局原位展开，不切换到独立编辑页面；编辑器可切换内容类型，保存后按新类型重新排入 AI 整理，同时遵循人工描述、人工标签优先规则。类型切换可从卡片右键菜单的“编辑信息”进入；“已处理 / 未处理”只参与侧栏筛选，不在卡片或详情中显示状态标识。
- 详情封面沿用共享封面比例：网站始终显示封面区，无图片时显示占位封面；文章仅当原网页提供封面图时显示，否则不渲染封面区；关注源保持原有图片形式，详情封面维持 `269 / 160` 原比例。
- 卡片使用 8px 内边缘留白；标题与描述内容区使用 4px 内边距，标签紧随描述并保持紧凑留白。网址、封面、信息和标签区块统一使用 8px 间距，标签之间也使用 8px 间距。
